...
start_input:
	...
	case 1:
		if (target_date <0 || target_date > 31){
	...
	case 2: // February 2000, a leap month, hence 29
		if (target_year % 4 != 0){ //not a leap year
			if (target_date <0 || target_date > 28){
				std::cout << "Error: Input date is outside of the range of the month." << std::endl;
				goto start_input;
			}
			else{
			...
			if (target_year % 100 == 0 && target_year % 400 != 0){ // not a leap year
				...
				if (target_date <0 || target_date > 29){
					std::cout << "Error: Input date is outside of the range of the month." << std::endl;
					goto start_input;
				}
	...
	case 4:
		if (target_date <0 || target_date > 30){
			std::cout << "Error: Input date is outside of the range of the month." << std::endl;
			goto start_input;
		}
		else{
	...
	if (target_month <= 2){
		for (int current_year = 2001; current_year < target_year + 1; current_year++){ // target_year +1 is such that the target_year itself is also considered
			...
			if ((current_year - 1) % 4 != 0){ //not a leap year
				...
				if ((current_year - 1) % 100 == 0 &&(current_year - 1) % 400 != 0){ // not a leap year
		...
		for (int current_year = 2001; current_year < target_year + 1; current_year++){ // target_year +1 is such that the target_year itself is also considered
			...
			if ((current_year) % 4 != 0){ //not a leap year
				...
				if ((current_year) % 100 == 0 &&(current_year) % 400 != 0){ // not a leap year
	...
	*/
	...
	if (target_date == 24 && target_month == 2){
